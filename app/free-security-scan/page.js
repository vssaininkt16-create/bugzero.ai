    ...
    <div className={`flex items-start gap-3 p-3 rounded-lg border ${check.pass ? 'border-green-500/20 bg-green-500/5' : 'border-red-500/20 bg-red-500/5'}`}>
      ...
      <div className="flex-1">
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium text-gray-900">{label || check.label}</span>
          <span className={`text-xs font-bold ${check.pass ? 'text-green-400' : 'text-red-400'}`}>
            {check.pass ? `+${check.score}pts` : '0pts'}
          </span>
        </div>
        <p className="text-xs text-gray-700 mt-0.5">{check.pass ? (check.value || check.details) : (check.recommendation || check.details)}</p>
      </div>
    </div>
  );
}
      ...
      <section className="relative py-20 sm:py-28 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 text-center">
          <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-green-50 border border-green-200 text-green-600 text-sm font-medium mb-6">
          ...
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
            ...
            Free Website <span className="text-red-600">Security Scan</span>
          </motion.h1>
            ...
            <div className="flex gap-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-4 rounded-xl bg-white border border-gray-200 shadow-sm border border-gray-200 text-gray-900 text-base placeholder-gray-500 focus:border-red-600 focus:ring-1 focus:ring-red-600 outline-none"
                  disabled={scanning}
                />
              </div>
              <button
                ...
                className="btn-primary px-6 py-4 text-base disabled:opacity-50 disabled:cursor-not-allowed shrink-0"
              >
        ...
        {scanning && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <div className="bg-white border border-gray-200 shadow-sm rounded-2xl p-12">
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-2">Scanning {url}...</h3>
              <p className="text-gray-700 text-sm">Checking SSL, security headers, DNS configuration...</p>
              <div className="mt-6 space-y-2 text-left max-w-xs mx-auto">
                {['SSL Certificate', 'HTTP Security Headers', 'DNS Records', 'Technology Detection'].map((c, i) => (
                  <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.5 }}
                    ...
                    <Loader2 className="w-3 h-3 text-red-600 animate-spin" /> {c}
                  </motion.div>
                ))}
              </div>
            </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="bg-white border border-gray-200 shadow-sm rounded-2xl p-8 mb-8">
              <div className="text-center mb-8">
                <h2 className="text-2xl font-bold text-gray-900 font-heading mb-2">Security Report for</h2>
                <p className="text-red-600 font-mono">{result.url}</p>
              </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="bg-white border border-gray-200 shadow-sm rounded-xl p-5">
                <h3 className="text-base font-bold text-gray-900 font-heading mb-3 flex items-center gap-2">
                  <Lock className="w-4 h-4 text-red-600" /> SSL Certificate
                </h3>
              ...
              <div className="bg-white border border-gray-200 shadow-sm rounded-xl p-5">
                <h3 className="text-base font-bold text-gray-900 font-heading mb-3 flex items-center gap-2">
                  <Server className="w-4 h-4 text-red-700" /> DNS Security
                </h3>
              ...
              <div className="bg-white border border-gray-200 shadow-sm rounded-xl p-5 md:col-span-2">
                <h3 className="text-base font-bold text-gray-900 font-heading mb-3 flex items-center gap-2">
                  <ShieldCheck className="w-4 h-4 text-green-600" /> HTTP Security Headers
                </h3>
              ...
              {result.checks?.headers?._tech && (
                <div className="bg-white border border-gray-200 shadow-sm rounded-xl p-5 md:col-span-2">
                  <h3 className="text-base font-bold text-gray-900 font-heading mb-3 flex items-center gap-2">
                    <Eye className="w-4 h-4 text-red-600" /> Technology Detection
                  </h3>
                  <div className="grid sm:grid-cols-2 gap-4 text-sm">
                    <div><span className="text-gray-600">Server:</span> <span className="text-gray-900 ml-2">{result.checks.headers._tech.server}</span></div>
                    <div><span className="text-gray-600">X-Powered-By:</span> <span className="text-gray-900 ml-2">{result.checks.headers._tech.poweredBy}</span></div>
                  </div>
                </div>
              )}
            </div>
            ...
            {/* CTA Section */}
            <div className="mt-8 bg-white border border-gray-200 shadow-sm rounded-2xl p-8 text-center">
              ...
              {showEmailForm && !emailSaved && (
                <form onSubmit={handleEmailSubmit} className="max-w-md mx-auto space-y-3 mt-4">
                  <input type="text" placeholder="Your Name" value={emailForm.name}
                    ...
                    className="w-full px-4 py-2.5 rounded-lg bg-white border border-gray-200 shadow-sm border border-gray-200 text-gray-900 text-sm focus:border-red-600 outline-none" />
            ...
            <div className="grid sm:grid-cols-3 gap-6">
              {[
                ...
                <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
                  ...
                  <item.icon className="w-10 h-10 text-red-600 mx-auto mb-3" />
                  <h3 className="text-base font-bold text-gray-900 font-heading mb-2">{item.title}</h3>
...
export default function SecurityScanPageWrapper() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-white flex items-center justify-center"><div className="w-8 h-8 border-2 border-cyber-blue border-t-transparent rounded-full animate-spin" /></div>}>
      <SecurityScanPage />
    </Suspense>
  );
}
