...
const RISK_COLORS = {
  High: { text: 'text-red-400', bg: 'bg-red-400/10', border: 'border-red-400/30', hex: '#f87171' },
  ...
  Informational: { text: 'text-gray-400', bg: 'bg-gray-400/10', border: 'border-gray-400/30', hex: '#9ca3af' },
};
...
export default function VaptReportPage() {
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-cyber-bg flex items-center justify-center px-4">
        <div className="max-w-md w-full cyber-card rounded-2xl p-8 text-center">
          <Lock className="w-12 h-12 text-red-400 mx-auto mb-4" />
          <h1 className="text-xl font-bold text-white mb-2">Access Denied</h1>
          <p className="text-cyber-muted text-sm mb-6">{error}</p>
  ...
  const gradeColor = { A: 'text-cyber-green', B: 'text-cyber-blue', C: 'text-yellow-400', D: 'text-red-400' }[grade] || 'text-red-400';
    ...
    <div className="min-h-screen bg-cyber-bg grid-bg">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex items-center justify-between mb-8">
          <Link href={`/vapt-dashboard`} className="flex items-center gap-1.5 text-cyber-muted hover:text-white text-sm transition-colors">
        ...
        {/* Report Header Card */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="cyber-card rounded-2xl p-6 mb-6">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <Shield className="w-5 h-5 text-cyber-blue" />
                <span className="text-xs text-cyber-blue font-semibold uppercase tracking-wider">VAPT Security Report</span>
              </div>
              <h1 className="text-xl font-bold text-white mb-1 break-all">{scan.target_url}</h1>
            ...
            <div className="text-center">
              ...
              <div className="text-sm text-cyber-muted">{score}/100</div>
              <div className="text-xs text-cyber-muted mt-0.5">Security Score</div>
            </div>
          </div>
        </motion.div>
        ...
        {/* Disclaimer */}
        <div className="flex items-start gap-3 cyber-card rounded-xl p-4 mb-6 border-yellow-400/20 bg-yellow-400/5">
        ...
        {vulns.length === 0 ? (
          <div className="cyber-card rounded-2xl p-8 text-center">
            ...
            <p className="text-cyber-muted text-sm">Your application passed all automated security checks.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {vulns.map((v, i) => {
              ...
              return (
                <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }} className="cyber-card rounded-xl overflow-hidden">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-2">
                          ...
                          {v.cwe && <span className="text-xs text-cyber-muted font-mono">{v.cwe}</span>}
                        </div>
                        <p className="text-sm text-cyber-muted mb-3">{v.description}</p>
                        {v.url && (
                          <p className="text-xs text-cyber-muted mb-2 font-mono truncate">URL: {v.url}</p>
                        )}
                        {v.evidence && (
                          <div className="bg-cyber-bg rounded-lg px-3 py-2 mb-3 font-mono text-xs text-gray-400 truncate">
                            {v.evidence}
                          </div>
                        )}
                        {v.recommendation && (
                          <div className="flex items-start gap-2 text-xs text-cyber-green bg-cyber-green/5 border border-cyber-green/20 rounded-lg px-3 py-2">
        ...
        {/* Footer */}
        <div className="mt-10 pt-8 border-t border-cyber-border text-center">
